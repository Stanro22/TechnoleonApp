...
struct VFMTTrial1: View {
    ...
    var body: some View {
        VStack{
            Spacer()
            VStack{
                Text(NSLocalizedString("Trial 1", comment: "Trial 1"))
                    .font(.custom("", size: 30))
            ...
            NavigationLink(destination: VFMTTrial2().onAppear{setVFMTBody()}) {
    ...
    func setVFMTBody(){
        VFMTBody.trial1_end_time = stopwatchManager.timeToSave
        VFMTBody.trail1_errors = errors
    }
}

struct VFMTTrial1_Previews: PreviewProvider {
